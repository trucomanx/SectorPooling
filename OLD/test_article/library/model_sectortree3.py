...
def create_model_sectortree3(file_of_weight='',input_shape=(224,224,3),nout=7,enable_summary=False, factor=0.5):
    ...
    modelo = tf.keras.Sequential([
        tf.keras.layers.Conv2D( 32, kernel_size= 9, padding="same", activation=FUNC_ACT, input_shape=input_shape),
        tf.keras.layers.Conv2D(  4, kernel_size= 9, padding="same", activation=FUNC_ACT),
        tf.keras.layers.MaxPooling2D( pool_size=(2, 2)),
        
        #
        
        tf.keras.layers.Conv2D( 32, kernel_size=7, padding="same", activation=FUNC_ACT),
        tf.keras.layers.Conv2D(  4, kernel_size=7, padding="same", activation=FUNC_ACT),
        #tf.keras.layers.MaxPooling2D( pool_size=(2, 2)),
        ...
        mst.model_sectortree2d( input_shape=(112,112,4),
        ...
        tf.keras.layers.Conv2D( 32, kernel_size=1, padding="same", activation=FUNC_ACT),
        tf.keras.layers.Conv2D( 16, kernel_size=1, padding="same", activation=FUNC_ACT),
        ...
        tf.keras.layers.Dense(nout*9+1,activation='tanh'),
